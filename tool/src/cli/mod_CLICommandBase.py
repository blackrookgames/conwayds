...
class CLICommandBase:
    ...
    def __init__(self):
        """
        Initializer for CLICommandBase
        """
    ...
    def __help(self, cmdname:None|str):
        ...
        def _print_col(_name:str, _short:None|str, _desc:None|str,\
            ...
            while not _end:
                ...
                _end = _stop >= len(_cast(str, _desc))
                if _end: _stop = len(_cast(str, _desc))
                print(f"{(' ' * _indent)}{_cast(str, _desc)[_start:_stop]}")
                _start += _descwidth
        ...
        for _param in self.__params.optparams:
            ...
            if namewidth < _len:
                namewidth = _len
            _len = 1 + len(_cast(str, _param.short))
            if shortwidth < _len:
                shortwidth = _len
        if self._help is not None:
        ...
        # Success!!!
        print()
        return self._PARSEARGS_HELP

    def _parseargs(self, argv:list[str], nohelp:bool):
        # No args (not even command name)
        if len(argv) == 0:
            if nohelp: return self._PARSEARGS_HELP
            return self.__help(None)
        # No args (just command name)
        if (not nohelp) and len(argv) == 1:
        ...
        for _i in range(len(self.__params.reqparams)):
            ...
            if not _s: return self._PARSEARGS_FAIL
